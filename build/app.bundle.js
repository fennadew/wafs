...
var helpers = exports.helpers = {
    htmlElement: function htmlElement(element) {
        return document.querySelector(element);
    },
    htmlElements: function htmlElements(elements) {
...
var collections = exports.collections = {
    all: [],
    map: function map() {
        var memeData = this.all.map(function (obj) {
            return {
    ...
    filter: function filter(name) {
        var memeData = this.map();
        var selectedMeme = memeData.filter(function (obj) {
            return obj.href === name;
        });
        return selectedMeme;
    },
    sort: function sort() {
        var memesAlphabetic = this.map();
        memesAlphabetic.sort(function (a, b) {
...
var sections = exports.sections = {
    ...
    toggle: function toggle(hash) {
        _helpers.helpers.htmlElements('section').forEach(function (el) {
...
var _transparency = __webpack_require__(336);

var _transparency2 = _interopRequireDefault(_transparency);

var _helpers = __webpack_require__(63);

var _collections = __webpack_require__(91);
...
var content = exports.content = {
    ...
    listPage: function listPage() {
        var data = void 0;
        if (_helpers.helpers.htmlElement('.sort input').checked) {
            data = _collections.collections.sort();
        } else {
            data = _collections.collections.map();
        }
        ...
        _transparency2.default.render(document.getElementById('memeslist'), data, directives);
    },

    //  Fills the detail page with content recieved from the API
    detailPage: function detailPage(name) {
        var directives = {
            image: {
        ...
        _transparency2.default.render(document.getElementById('details'), _collections.collections.filter(name), directives);
    }
};
...
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(92), __webpack_require__(329)(module)))
...
var _sections = __webpack_require__(127);
...
var _api = __webpack_require__(339);
...
var routes = exports.routes = {
    init: function init() {
        (0, _route2.default)({
            ...
            'memes': function memes() {
                // Checks if data is already there and if not, do API request
                if (_collections.collections.all.length === 0) {
                    _api.api.requestApi();
                } else {
                    _content.content.listPage();
                }

                _helpers.helpers.htmlElement('.sort input').addEventListener('change', _content.content.listPage);
                _sections.sections.toggle('memes');
            },
            'memes/:name': function memesName(name) {
                // Checks if data is already there and if not, do API request
                if (_collections.collections.all.length === 0) {
                    _api.api.requestApi(name);
                } else {
                    _content.content.detailPage(name);
                }
                _sections.sections.toggle('details');
            }
        });
    }
};
...
var api = exports.api = {
    ...
    requestApi: function requestApi(name) {
        ...
        var reqSettings = new Request('https://apifenna.casburggraaf.com/Generators_Select_ByPopular?pageIndex=' + this.pageNumberByDay + '&pageSize=12&days=&apiKey=' + this.apiKey, {
            method: 'get',
            headers: new Headers({
                    ...
                    while (1) {
                        switch (_context.prev = _context.next) {
                            ...
                            case 6:
                                ...
                                _collections.collections.all = data.result;
                                _helpers.helpers.htmlElement('.loader').classList.remove('active');

                                if (_helpers.helpers.htmlElement('.error').classList.contains('active')) {
                                    _helpers.helpers.htmlElement('.error').classList.remove('active');
                                }
